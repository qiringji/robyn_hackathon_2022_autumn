...
source('../../data_gen/functions/_step_9_calculater_roi.R')
...
dgp_name <- 'RQ2_dgp5_1000_3_01'
revenue_per_conv <- 1

####### Stage1. Data Generation: DGP No.3 #######
y_variables <- step_0_define_basic_parameters(
  years = 5,
...
df_ads_step3 <- step_3_generate_media(
  ...
  std_noisy_cpm_cpc = c(0.001, 0.015)
)
df_ads_step3
...
for (t in c(2, 5, 10, 3, 50, 4, 8, 7, 260)){
for (use_prophet in c(TRUE)) {
  for (calib in list(c("spend_Facebook", "spend_TV"))) {
    ...
    robyn_object <- paste("~/Desktop/", paste(dgp_name, "/", sep=""), "prophet_", use_prophet, "_calib_", paste(calib, collapse="_"), "_", t, sep="")
    print(robyn_object)
    ...
    df_range <- head(df_final_cv, t)
    
    calib_lift_f <- sum(df_range[c("conv_Facebook")]) * revenue_per_conv
    calib_spend_f <- sum(df_range[c("spend_Facebook")] + rnorm(t, mean = 0, sd = 200))
    calib_lift_t <- sum(df_range[c("conv_TV")]) * revenue_per_conv
    calib_spend_t <- sum(df_range[c("spend_TV")] + rnorm(t, mean = 0, sd = 200))
    ...
    if (!is.null(calib)) {
      ...
      calibration_input <- data.frame(
        ...
        liftStartDate = as.Date(rep(min(df_range$DATE), each=length(calib))),
        # liftEndDate must be within input data range
        liftEndDate = as.Date(rep(max(df_range$DATE), each=length(calib))),
