...
dgp_name <- 'dgp3_1000_3_01_3media_5cluster'
revenue_per_conv <- 1

my_variables <- step_0_define_basic_parameters(
  years = 5,
  channels_impressions = c("Facebook", "TV", "Search"),
  channels_clicks = c(),
  frequency_of_campaigns = 1,
  true_cvr = c(0.05, 0.02, 0.01),
  revenue_per_conv = 1
)
...
df_ads_step2 <- step_2_ads_spend_biased(
  my_variables = my_variables,
  spend_base <- c(100000, 200000, 120000),
  spend_trend <- c(0.0, 0.0, 0.0),
  spend_temp_var <- c(0.0, 0.0, 0.0),
  spend_temp_coef_mean <- c(5000, 7000, 3000),
  spend_temp_coef_sd <- c(500, 700, 400),
  spend_error_std <- c(25000, 50000, 30000),
  spend_season_diff_week <- c(0, 0, 0)
)
...
df_ads_step3 <- step_3_generate_media(
  ...
  true_cpm = c(10, 10, 10),
  true_cpc = c(),
  mean_noisy_cpm_cpc = c(1, 0.05, 1),
  std_noisy_cpm_cpc = c(0.01, 0.15, 0.05)
)
df_ads_step3
...
df_ads_step4 <- step_4_generate_cvr(
  ...
  mean_noisy_cvr = c(0, 0.000, 0),
  std_noisy_cvr = c(0.0001, 0.0001, 0.0001)
)
...
df_ads_step5c <- step_5c_diminishing_returns(
  ...
  gamma_saturation = c(0.2, 0.3, 0.2)
)
...
df_final$se_incremental <-
  df_final$baseline_sales + df_ads_step7$conv_Search
df_final %>%
  select(.,
         c(
           DATE,
...
df_final_cv = dplyr::inner_join(df_final, df_ads_step7[, c("DATE", "conv_Facebook", "conv_TV", "conv_Search")], by='DATE')
...
for (use_prophet in c(TRUE)) {
  for (calib in list(NULL, c("spend_Facebook"), c("spend_TV"), c("spend_Search"), c("spend_Facebook", "spend_TV"), c("spend_Facebook", "spend_Search"), c("spend_Search", "spend_TV"), c("spend_Facebook", "spend_TV", "spend_Search"))) {
    ...
    InputCollect <- robyn_inputs(
      ...
      paid_media_spends = c("spend_Facebook", "spend_TV", 'spend_Search'), # mandatory input
      paid_media_vars = c("impressions_Facebook", "impressions_TV", "impressions_Search"), # mandatory.
    ...
    df_range <- df_final_cv %>% filter(DATE >= "2018-07-01" & DATE <= "2018-10-07")
    df_range
    ...
    calib_lift_s <- sum(df_range[c("conv_Search")]) * revenue_per_conv
    calib_spend_s <- sum(df_range[c("spend_Search")])
    
    if (!is.null(calib)) {
      # TODO: Glue code.
      if (length(calib) == 3) {
        # c("spend_Facebook", "spend_Search", "spend_TV")
        liftAbs <- c(calib_lift_f, calib_lift_t, calib_lift_s)
        spend <- c(calib_spend_f, calib_spend_t, calib_spend_s)
      } else if (length(calib) == 2) {
        if (any(calib == "spend_Facebook") && any(calib == "spend_TV")) {
          # c("spend_Facebook", "spend_TV")
          liftAbs <- c(calib_lift_f, calib_lift_t)
          spend <- c(calib_spend_f, calib_spend_t)
        } else if (any(calib == "spend_Facebook")) {
          # c("spend_Facebook", "spend_Search")
          liftAbs <- c(calib_lift_f, calib_lift_s)
          spend <- c(calib_spend_f, calib_spend_s)
        } else {
          # c("spend_Search", "spend_TV")
          liftAbs <- c(calib_lift_s, calib_lift_t)
          spend <- c(calib_spend_s, calib_spend_t)
        }
      } else {
        if (any(calib == "spend_Facebook")) {
          # c("spend_Facebook")
          liftAbs <- c(calib_lift_f)
          spend <- c(calib_spend_f)
        } else if (any(calib == "spend_TV")) {
          # c("spend_TV")
          liftAbs <- c(calib_lift_t)
          spend <- c(calib_spend_t)
        } else {
          # c("spend_Search")
          liftAbs <- c(calib_lift_s)
          spend <- c(calib_spend_s)
        }
      }
